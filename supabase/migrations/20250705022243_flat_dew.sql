/*
  # Fix Returns System Credit Account Error
  
  1. Changes
    - Modify process_return function to handle missing cuentas_credito table
    - Add conditional check before attempting to update credit account
    - Improve error handling in return processing
    - Fix notification system for returns
*/

-- Enhance the process_return function to handle missing credit accounts
...
BEGIN
  ...
  IF NEW.estado = 'aprobada' AND (OLD.estado IS NULL OR OLD.estado != 'aprobada') THEN
    ...
    -- This prevents errors if the table doesn't exist in the database
    SELECT EXISTS (
    ...
    -- If it was a credit sale and cuentas_credito table exists, adjust credit balance if needed
    IF v_metodo_pago = 'credito' AND v_cliente_id IS NOT NULL AND v_cuenta_credito_exists THEN
      BEGIN
        -- Update credit account if exists - wrapped in exception block for safety
...
-- Add a function to notify admins when a new return is created with better error handling
...
BEGIN
  ...
  FOR v_admin_id IN (
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      RAISE NOTICE 'Error creating notification for admin %: %', v_admin_id, SQLERRM;
    END;
  END LOOP;
