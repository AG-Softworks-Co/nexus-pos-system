/*
  # Sistema de Configuración Completo - CORREGIDO
...
-- Función CORREGIDA para asegurar que solo hay un tema/plantilla activa por negocio
CREATE OR REPLACE FUNCTION ensure_single_active()
RETURNS TRIGGER AS $$
BEGIN
  -- Para temas personalizados (usa es_activo)
  IF TG_TABLE_NAME = 'temas_personalizados' AND NEW.es_activo = true THEN
    UPDATE temas_personalizados 
    SET es_activo = false 
    WHERE negocio_id = NEW.negocio_id AND id != NEW.id;
  END IF;
  
  -- Para plantillas de ticket (usa es_activa)
  IF TG_TABLE_NAME = 'plantillas_ticket' AND NEW.es_activa = true THEN
