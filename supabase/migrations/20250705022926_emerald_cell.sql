/*
  # Fix Returns System and Credit Account Error
  
  1. Changes
    - Fix process_return function to handle missing cuentas_credito table
    - Add better error handling for credit account operations
...
BEGIN
  ...
  IF NEW.estado = 'aprobada' AND (OLD.estado IS NULL OR OLD.estado != 'aprobada') THEN
    ...
    BEGIN
      ...
      END IF;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Error checking cuentas_credito table: %', SQLERRM;
    END;
  END IF;
