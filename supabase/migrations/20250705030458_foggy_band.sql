/*
  # Fix Dashboard Stats and Return Handling
  
  1. Changes
    - Add function to calculate correct percentage changes
    - Update detalle_ventas to track returned quantities
    - Ensure PDF receipts show returned items correctly
    - Fix sales table to show return status
*/

-- Add column to track returned quantities in detalle_ventas if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Update the process_return function to mark items as returned
...
BEGIN
  ...
  IF NEW.estado = 'aprobada' AND (OLD.estado IS NULL OR OLD.estado != 'aprobada') THEN
    BEGIN
      ...
      -- Process returned items and update detalle_ventas
      BEGIN
        FOR v_detalle IN (
          SELECT 
            dd.detalle_venta_id,
          ...
          -- Update detalle_ventas to mark items as returned
          UPDATE detalle_ventas
          SET cantidad_devuelta = cantidad_devuelta + v_detalle.cantidad_devuelta
          WHERE id = v_detalle.detalle_venta_id;
        ...
        END LOOP;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Error processing returned items: %', SQLERRM;
      END;
...
-- Create a function to get return information for a sale
CREATE OR REPLACE FUNCTION get_sale_return_info(p_sale_id UUID)
RETURNS TABLE (
  return_id UUID,
...
BEGIN
  ...
  WHERE d.venta_id = p_sale_id
  AND d.estado = 'aprobada';
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Update existing returns to set cantidad_devuelta in detalle_ventas
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN (
    ...
    UPDATE detalle_ventas
    SET cantidad_devuelta = r.cantidad_devuelta
