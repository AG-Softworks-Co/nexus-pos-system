/*
  # Sistema de Configuración Completo - VERSIÓN FINAL CORREGIDA
...
-- PRIMERO: Eliminar función existente que puede estar causando conflictos
DROP FUNCTION IF EXISTS ensure_single_active() CASCADE;
...
-- Función SEPARADA para temas personalizados
...
-- Función SEPARADA para plantillas de ticket
...
-- Triggers SEPARADOS para cada tabla
...
-- Insertar configuración por defecto para negocios existentes
DO $$
DECLARE
  negocio_record RECORD;
BEGIN
  FOR negocio_record IN SELECT id FROM negocios LOOP
    -- Insertar configuración si no existe
    INSERT INTO configuracion_negocio (negocio_id) 
    VALUES (negocio_record.id)
    ON CONFLICT (negocio_id) DO NOTHING;
    
    -- Insertar plantilla por defecto si no existe
    INSERT INTO plantillas_ticket (negocio_id, nombre, descripcion, es_activa)
    VALUES (negocio_record.id, 'Plantilla Estándar', 'Plantilla por defecto', true)
    ...
    VALUES (negocio_record.id, 'Tema Nexus', 'Tema por defecto', true, '#6366f1', '#8b5cf6')
    ON CONFLICT DO NOTHING;
  END LOOP;
END $$;
